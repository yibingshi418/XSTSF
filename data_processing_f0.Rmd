...
data_merge <- function(df) {
  ...
  selected_cols <- intersect(c('token', 'syntax', 'token_no', 'speaker', 'citation_tone', 'tone', 'mono_tone_1'), colnames(df_merge))
...
f0_all_pre <- 
  ...
  mutate(focus_condition = coalesce(focus_condition, 'citation'),
         ind_no = ifelse(focus_condition == 'citation', paste(speaker, citation_no, sep = '_'),
