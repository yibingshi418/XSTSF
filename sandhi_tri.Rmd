...
# plotting
p_cluster_cont(f0_di_ct_yp, sandhi_tone)+
...
distri_prop2(f0_yp_comp1, diortri, sandhi_category)+
  scale_fill_manual(values = c("#4477AA", "#CC6677", "#DDCC77", "#66CCEE", "#9B72C7"))+
...
funciton
```{r}
format_axis_labels <- function(labels, label_colors) {
  sapply(labels, function(label) {
    # Split the label into parts before and after the "_"
    parts <- unlist(strsplit(label, "_"))
    if (length(parts) == 2) {
      # Color the part before "_" based on the label_colors mapping
      paste0(
        "<span style='color:", label_colors[parts[1]], ";'>", parts[1], "</span>_", parts[2]
      )
    } else {
      # If no "_" is present, return the label as is
      label
    }
  })
}

# Define a mapping of the tones to their colours
label_colors <- c("Ia" = "blue", "Ib" = "red", "IIa" = "green", 
                  "IIb" = "purple", "IIIa" = "orange", "IIIb" = "brown")
```
...
f0_tri_ct_yp %>% distri_prop2(syntax, sandhi_tone)+
  scale_fill_manual(values = c("#4477AA", "#66CCEE", "#DDCC77", "#CC6677", "#9B72C7"))+
  labs(title = "Distribution by syntax")
...
f0_yp_comp1 %>% 
  ...
  scale_fill_manual(values = c("#4477AA", "#CC6677", "#DDCC77", "#66CCEE", "#9B72C7"))+
  labs(title = "Distribution by speaker in trisyllables")
f0_yp_comp1 %>% 
  filter(diortri == 'disyllable') %>% 
  distri_prop2(speaker, sandhi_category, label_format = "paste0(round(prop * 100), '%')")+
  scale_fill_manual(values = c("#4477AA", "#CC6677", "#DDCC77"))+
  labs(title = "Distribution by speaker in disyllables")
```
